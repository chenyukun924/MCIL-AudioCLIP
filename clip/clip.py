...
def _convert_to_rgb(image):  #修改过
    if isinstance(image, torch.Tensor):
        image = to_pil_image(image)
    return image.convert('RGB')

def _transform(n_px: int, is_train: bool):  #修改过
    normalize = Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))
    if is_train:  #训练集预处理  train
        return Compose([
            RandomResizedCrop(n_px, scale=(0.9, 1.0), interpolation=InterpolationMode.BICUBIC),
            _convert_to_rgb,
            ToTensor(),
            normalize,
            #Normalize((0.35477436, 0.32282564, 0.31845313), (0.23847212, 0.22961944, 0.2260488)),  #classroom_15
            #Normalize((0.49596068, 0.47724837, 0.42231348), (0.23020183, 0.22948943, 0.22636114)),  #imagenet18
        ])
        ...
        return Compose([
            ...
            ToTensor(),
            normalize,
            #Normalize((0.35286754, 0.3210081,  0.31634438), (0.23888503, 0.2298958,  0.22611365)),  #classroom_15
            #Normalize((0.49127352, 0.46561578, 0.41488686), (0.24201028, 0.23747574, 0.2318833)),  #imagenet18
        ])
